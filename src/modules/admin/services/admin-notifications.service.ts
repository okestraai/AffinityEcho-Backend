...
export class AdminNotificationsService {
  ...
  async listNotifications(query: {
    ...
    let q = this.admin
      ...
      .select('*, creator:user_profiles!created_by(id, username)', {
        count: 'exact',
      })
    ...
    // Summary
    const { data: allRows } = await this.admin
      .from('admin_notifications')
      .select('status, recipients_count');
    const summary = { sent: 0, scheduled: 0, draft: 0, total_reach: 0 };
    for (const r of allRows ?? []) {
      if (r.status === 'sent') {
        summary.sent++;
        summary.total_reach += r.recipients_count ?? 0;
  ...
  async createNotification(
    ...
    if (action === 'schedule' && !scheduled_at)
      throw new BadRequestException(
        'scheduled_at required when action is schedule',
      );
    ...
    if (action === 'send') {
      recipientsCount = await this.fanOut(
        adminId,
        notif.id,
        title,
        message,
      ...
      await this.admin
        .from('admin_notifications')
        .update({ recipients_count: recipientsCount })
        .eq('id', notif.id);
    }
    ...
    await this.adminUsers.logAction(
      ...
      notif.id,
      undefined,
      { audience, action, recipients_count: recipientsCount },
      ip,
    );

    return {
      success: true,
      data: { ...notif, recipients_count: recipientsCount },
    };
  }
  ...
  async sendNotification(
    ...
    const recipientsCount = await this.fanOut(
      adminId,
      notifId,
      notif.title,
      notif.message,
    ...
    return {
      success: true,
      data: { id: notifId, status: 'sent', recipients_count: recipientsCount },
    };
  }
  ...
  async deleteNotification(
    ...
    const { data: notif } = await this.admin
      .from('admin_notifications')
      .select('status')
      .eq('id', notifId)
      .single();
    ...
    if (notif.status === 'sent')
      throw new UnprocessableEntityException(
        'Cannot delete a sent notification',
      );
  ...
  async exportNotifications(
    ...
    let q = this.admin
      .from('admin_notifications')
      .select(
        'id, title, message, type, audience, status, recipients_count, sent_at, scheduled_at, created_at',
      )
      .order('created_at', { ascending: false });
    ...
    const buffer = await new Promise<Buffer>((resolve, reject) => {
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#ffffff')
        .text('Notifications Export', m, 18);
      doc
        .fontSize(9)
        .font('Helvetica')
        .fillColor('#bae6fd')
        .text(
          `Generated: ${new Date().toLocaleString()}  |  Total: ${notifs.length}`,
          m,
          40,
        );
      ...
      notifs.forEach((n: any, i: number) => {
        ...
        doc
          .rect(m, y, pw - 2 * m, 20)
          .fill(i % 2 === 0 ? '#ffffff' : '#f0f9ff');
        let x = m;
        doc.fontSize(7).fillColor('#1f2937');
        doc
          .font('Helvetica-Bold')
          .text(n.title || '-', x + 4, y + 6, { width: cols[0].w - 8 });
        x += cols[0].w;
        doc
          .font('Helvetica')
          .text(n.type || '-', x + 4, y + 6, { width: cols[1].w - 8 });
        x += cols[1].w;
        doc.text(n.audience || 'all', x + 4, y + 6, { width: cols[2].w - 8 });
        x += cols[2].w;
        doc
          .fillColor(statusColors[n.status] || '#374151')
          .font('Helvetica-Bold')
          .text((n.status || '-').toUpperCase(), x + 4, y + 6, {
            width: cols[3].w - 8,
          });
        x += cols[3].w;
        doc
          .fillColor('#1f2937')
          .font('Helvetica')
          .text(String(n.recipients_count ?? 0), x + 4, y + 6, {
            width: cols[4].w - 8,
          });
        x += cols[4].w;
        doc.text(
    ...
    return {
      buffer,
      filename: `notifications-${date}.pdf`,
