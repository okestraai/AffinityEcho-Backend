...
describe('UserBlockingService', () => {
  ...
  describe('blockUser', () => {
    it('should block user successfully', async () => {
      const userChain = createMockQueryChain({ data: { id: 'u2', username: 'Test' }, error: null });
    ...
    it('should throw if blocking self', async () => {
      await expect(service.blockUser('u1', 'u1', {}))
        .rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if already blocked', async () => {
      const userChain = createMockQueryChain({ data: { id: 'u2', username: 'Test' }, error: null });
      const existsChain = createMockQueryChain({ data: { id: 'existing' }, error: null });
      mockClient.from.mockReturnValueOnce(userChain).mockReturnValueOnce(existsChain);

      await expect(service.blockUser('u1', 'u2', {}))
        .rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('unblockUser', () => {
    it('should unblock user successfully', async () => {
      const findChain = createMockQueryChain({ data: { id: 'block-1' }, error: null });
      const deleteChain = createMockQueryChain({ data: null, error: null });
      mockClient.from.mockReturnValueOnce(findChain).mockReturnValueOnce(deleteChain);
    ...
    it('should throw NotFoundException if not blocked', async () => {
      ...
      await expect(service.unblockUser('u1', 'u2'))
  ...
  describe('getBlockedUsers', () => {
    it('should return formatted blocked users', async () => {
      const blocksChain = createMockQueryChain({
        data: [{ id: 'b1', blocked_id: 'u2', reason: 'spam', created_at: '2026-01-01' }],
        error: null,
        count: 1,
      });
      ...
      mockClient.from.mockReturnValueOnce(blocksChain).mockReturnValueOnce(usersChain);
  ...
  describe('getBlockStatus', () => {
    it('should return isBlocked true when user blocked target', async () => {
      const blockedByMe = createMockQueryChain({ data: { id: 'b1' }, error: null });
      const blockedByThem = createMockQueryChain({ data: null, error: null });
      mockClient.from.mockReturnValueOnce(blockedByMe).mockReturnValueOnce(blockedByThem);
    ...
    it('should return isBlockedBy true when target blocked user', async () => {
      ...
      const blockedByThem = createMockQueryChain({ data: { id: 'b1' }, error: null });
      mockClient.from.mockReturnValueOnce(blockedByMe).mockReturnValueOnce(blockedByThem);
