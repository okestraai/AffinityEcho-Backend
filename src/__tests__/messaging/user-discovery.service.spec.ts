...
describe('UserDiscoveryService', () => {
  ...
  const mockUser = {
    id: 'u2', username: 'User2', avatar: '📚', job_title: 'Designer',
    company_encrypted: 'enc_Google', career_level_encrypted: 'enc_mid',
    bio: 'hello', skills: ['design'], location: 'NYC',
    is_company_verified: true, mentoring_as: 'mentor',
    is_active_mentor: true, is_active_mentee: false,
    privacy_level: 'public', created_at: '2026-01-01', last_active_at: '2026-05-01',
  };
  ...
  beforeEach(() => {
    ...
    service = new UserDiscoveryService(createMockConfigService() as any, mockEncryption);
  });

  describe('getConnectableUsers', () => {
    it('should return connectable users', async () => {
      const usersChain = createMockQueryChain({ data: [mockUser], error: null });
      const convsChain = createMockQueryChain({ data: [], error: null });
      const mutualChain = createMockQueryChain({ data: null, error: null, count: 2 });
    ...
    it('should exclude existing conversations when requested', async () => {
      const usersChain = createMockQueryChain({ data: [mockUser, { ...mockUser, id: 'u3' }], error: null });
      ...
      const result = await service.getConnectableUsers('u1', { exclude_existing: true });
      expect(result.success).toBe(true);
    });
    ...
    it('should decrypt company and career_level', async () => {
      const usersChain = createMockQueryChain({ data: [mockUser], error: null });
      const convsChain = createMockQueryChain({ data: [], error: null });
      const mutualChain = createMockQueryChain({ data: null, error: null, count: 0 });
  ...
  describe('searchUsers', () => {
    it.skip('should search users by query', async () => {
      ...
      const result = await service.searchUsers('u1', 'Designer');
      expect(result.success).toBe(true);
    });
    ...
    it.skip('should return empty for no matches', async () => {
      ...
      const result = await service.searchUsers('u1', 'nonexistent');
      expect(result.success).toBe(true);
