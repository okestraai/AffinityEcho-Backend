...
import { ContentSafetyService } from '../../modules/content-safety/content-safety.service';
...
describe('ContentSafetyService', () => {
  ...
  describe('flagContent', () => {
    ...
    it('should throw ConflictException if duplicate', async () => {
      ...
      await expect(
        service.flagContent(reporterId, contentType, contentId, dto),
  ...
  describe('hideContent', () => {
    ...
    it('should throw on upsert error', async () => {
      ...
      await expect(
        service.hideContent('user-1', 'post', 'post-1'),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getHiddenContentIds', () => {
    it('should return content IDs', async () => {
      const chain = createMockQueryChain({
        data: [{ content_id: 'a' }, { content_id: 'b' }],
        error: null,
      });
      mockClient.from.mockReturnValueOnce(chain);
  ...
  describe('getBlockedUserIds', () => {
    it('should return IDs from both directions', async () => {
      const byMe = createMockQueryChain({
        data: [{ blocked_id: 'u2' }],
        error: null,
      });
      const byThem = createMockQueryChain({
        data: [{ blocker_id: 'u3' }],
        error: null,
      });
  ...
  describe('getFlags', () => {
    it('should return paginated flags with profiles', async () => {
      const flagsChain = createMockQueryChain({
        data: [
          { id: 'f1', reporter_id: 'u1', reviewed_by: null, status: 'pending' },
        ],
        error: null,
        count: 1,
      });
    ...
    it('should throw on error', async () => {
      ...
      await expect(service.getFlags({ page: 1, limit: 20 })).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('reviewFlag', () => {
    ...
    it('should throw on invalid status', async () => {
      await expect(
        service.reviewFlag('f1', 'invalid', 'admin-1'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw on update error', async () => {
      ...
      await expect(
        service.reviewFlag('f1', 'reviewed', 'admin-1'),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
