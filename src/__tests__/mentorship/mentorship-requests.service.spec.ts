...
describe('MentorshipRequestsService', () => {
  ...
  describe('sendDirectRequest', () => {
    it.skip('should send a mentor request', async () => {
      ...
      const existPendingChain = createMockQueryChain({
        data: null,
        error: null,
      });
      const existAcceptedChain = createMockQueryChain({
        data: null,
        error: null,
      });
      // 2. insert request
      ...
      // 3. notification
      const notifProfileChain = createMockQueryChain({
        data: { username: 'Mentee' },
        error: null,
      });
      const notifInsertChain = createMockQueryChain({
        data: null,
        error: null,
      });
    ...
    it.skip('should throw if target user not found', async () => {
      const targetChain = createMockQueryChain({
        data: null,
    ...
    it.skip('should throw if target is deactivated', async () => {
      const targetChain = createMockQueryChain({
    ...
    it.skip('should throw on duplicate request (23505)', async () => {
      const targetChain = createMockQueryChain({
        data: mockTargetUser,
        error: null,
      });
  ...
  describe('getDirectRequests', () => {
    it.skip('should return received requests', async () => {
      ...
      const result = await service.getAllDirectRequestsForUser('u2', {
        type: 'received',
      } as any);
      expect(result.success).toBe(true);
    ...
    it.skip('should return sent requests', async () => {
      const chain = createMockQueryChain({
        data: [mockRequest],
        error: null,
        count: 1,
      ...
      const result = await service.getAllDirectRequestsForUser('u1', {
        type: 'sent',
      } as any);
      expect(result.success).toBe(true);
    });
  ...
  describe('respondToDirectRequest', () => {
    it.skip('should accept a request', async () => {
      ...
      const result = await service.respondToDirectRequest('u2', {
        requestId: 'req-1',
        action: 'accept',
      } as any);
      expect(result.success).toBe(true);
    ...
    it.skip('should throw if request not found', async () => {
      ...
      await expect(
        service.respondToDirectRequest('u2', {
          requestId: 'nope',
          action: 'accept',
        } as any),
      ).rejects.toThrow(NotFoundException);
    ...
    it.skip('should throw if request not pending', async () => {
      const chain = createMockQueryChain({
  ...
  describe('getRequestMetrics', () => {
    it.skip('should return request metrics', async () => {
  ...
  describe('markRequestAsRead', () => {
    it.skip('should mark request as read', async () => {
      const requestChain = createMockQueryChain({
        data: { id: 'req-1', target_user_id: 'u2' },
        error: null,
      });
    ...
    it.skip('should throw if request not found', async () => {
      ...
      await expect(
        service.markDirectRequestAsRead('u2', 'nope'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getRequestById', () => {
    ...
    it.skip('should throw if not found', async () => {
      ...
      await expect(service.getDirectRequest('u1', 'nope')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
